...
static void gc_mark_node(ds_node_t node) {
  ...
  gc_mark_node(sub_fields[0]);
  gc_mark_node(sub_fields[1]);
}
...
void ds_arena_run_gc(_ds_arena_t_ *a) {
  ...
  while (*curr) {
    ...
    if (!track->marked) {
      // SAUVEGARDE : On extrait l'élément suivant avant que la structure ne soit détruite
      _ds_allocation_track_t_ *next_track = track->next;

      // Recyclage : va appeler ds_gc_unregister_allocation et libérer proprement 'track'
      ds_arena_recycle(a, track->ptr);

      // Mise à jour sûre du maillon de la liste
      *curr = next_track;
      recycled_count++;
    } else {
