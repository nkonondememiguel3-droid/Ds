...
static inline _ds_arena_chunk_t_ *ds_arena_grow(_ds_arena_t_ *a,
                                                size_t needed) {
  size_t sz = (a->chunk_size > needed) ? a->chunk_size : needed;
  sz = ds_arena_align_up(sz);

  _ds_arena_chunk_t_ *c =
      (_ds_arena_chunk_t_ *)malloc(sizeof(_ds_arena_chunk_t_) + sz);
  if (!c) {
...
inline void *ds_arena_alloc(_ds_arena_t_ *a, size_t size) {
  ...
  if (!c || c->chunk_size_used + size > c->chunk_size)
    c = ds_arena_grow(a, size);
