...
// 3. BENCHMARK : LISTE CHAÎNÉE CIRCULAIRE
void bench_linked_list(size_t elements) {
  ...
  size_t limit = elements / 2;
  for (size_t i = 0; i < limit; i++) {
    if (!curr || list->length <= 1) break;
    ds_list_node_t *survivor = curr->next;
    ds_list_node_t *next_to_process = survivor->next;
